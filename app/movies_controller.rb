...
def can_find_using_where_clause_and_be_sorted
  ...
  ActiveRecord::Base.connection.execute("SELECT title FROM movies WHERE release_date>2002 ORDER BY release_date DESC").map {|m| m[title]}
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick"
  Movie.create(title: "Awesome Flick")
  movie=Movie.find_by(title: "Awesome Flirk")
  movie.title="Even Awesomer Flirk"
  movie.update
end
